...
try:
    ...
    for i in range(iters):
        ...
        for rtt in [50]:
            for cong in ["bbr2"]:  # reintroduce "reno"
                for scenario in [constantins.SCENARIO.INCUMBENT_CALL]:
                    for mvca in mvcas:
                        bw = bwdict[mvca]
                        bdp = float(rtt * bw * 10.0 ** 3) / 8.0

                        for scalar in [10.0]:
                            fifodisc = QDiscConfig("bfifo", int(ceil(scalar * bdp)))
                        ...
                        for aqm in [constantins.CODEL, constantins.FQCODEL]:
                            # Do Codel Measurements
                            pass
                            # scenarios.evaluate_scenario(constantins.TestConfig("<invalidguid>", scenario, mvca, rtt, bw, aqm, cong))
